...
def verify_base_src():
    try:
        ...
        try:
            ...
            logger.log(level=logging.INFO, msg='Unable to find file for building base sources.')
...
def req_country_src_data(alpha2_code, src_cat=None):
    ...
    try:
        ...
        logger.log(level=logging.ERROR, msg=f'{err} in req_country_src_data({alpha2_code}, {src_cat})')
        return None
...
def check_for_source(src_name: str) -> Source or None:
    if src_name:
        try:
            ...
            logger.log(
                level=logging.ERROR,
                msg=f'{err} in check_for_source({src_name})'
            )
            return None
    else:
        return None
